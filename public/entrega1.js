...
function init() {
  ...
  loader.load("map.osm", function (text) {   
    ...
    for (let i = 0; i < x.length; i++) {
      ...
      for (let j = 0; j < tags.length; j++) {
        if (tags[j].hasAttribute("k")) {
         ...
         if (tags[j].getAttribute("k") == "amenity") {
            interest = 3;break;
         }
        }
      }
      ...
      if (interest > 0) {
        ...
        if (interest == 1) {
        ...
        else if (interest == 3) { // Amenity
          const shape = new THREE.Shape();
          shape.autoClose = true;

          // Recorrer las etiquetas para encontrar el tipo de amenity
          let amenityType = "unknown"; // Valor predeterminado si no se encuentra la etiqueta
          for (let j = 0; j < tags.length; j++) {
            if (tags[j].hasAttribute("k") && tags[j].getAttribute("k") === "amenity") {
              amenityType = tags[j].getAttribute("v");
              break; // No es necesario seguir buscando después de encontrar la etiqueta "amenity"
            }
          }
          ...
          // Asignar colores basados en el tipo de amenity
          let amenityMaterial;
          if (amenityType === "toilets") {
            amenityMaterial = new THREE.LineBasicMaterial({ color: 0x8B4513 }); // Marrón
          } else if (amenityType === "bar") {
            amenityMaterial = new THREE.LineBasicMaterial({ color: 0xD2691E }); // Chocolate
          } else if (amenityType === "parking") {
            amenityMaterial = new THREE.LineBasicMaterial({ color: 0xA52A2A }); // Marrón Rojizo
          } else if (amenityType === "charging_station") {
            amenityMaterial = new THREE.LineBasicMaterial({ color: 0x8B4513 }); // Marrón otro estilo de marron
          }else if (amenityType === "restaurant") {
            amenityMaterial = new THREE.LineBasicMaterial({ color: 0x8B4513 }); // Marrón otro estilo de marron
          } else {
            amenityMaterial = new THREE.LineBasicMaterial({ color: 0x8B4513 }); // Otro color por defecto
          }

          const line = new THREE.Mesh(geometry, amenityMaterial);
          scene.add(line);
        }
      }
    }
